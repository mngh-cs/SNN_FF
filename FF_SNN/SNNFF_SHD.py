...
class LeakyLayer(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        ...
        self.lif1 = snn.RLeaky(beta=beta, threshold=threshold, 
                               spike_grad=spike_grad4, linear_features=1000, learn_threshold=True, learn_beta=True)
        self.layer_outputs = []
...
class LeakyLayer2(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        ...
        self.lif2 = snn.RLeaky(beta=beta, threshold=threshold, 
                               spike_grad=spike_grad4, linear_features=1000, learn_threshold=True, learn_beta=True)
        self.layer_outputs = []
        self.num_steps = num_steps
...
train_set = tonic.datasets.SHD(save_to='./data', transform=frame_transform, train=True)
test_set = tonic.datasets.SHD(save_to='./data', transform=frame_transform, train=False)
...
def make_examples(model, x, y_true, epsilon=1e-12):
    ...
    y_hard = torch.multinomial(g.sqrt() + epsilon, 1).squeeze(1)
...
# %%
def swish_loss(h_pos, h_neg, alpha=4.0):
    g_pos, g_neg = goodness(h_pos), goodness(h_neg)
    Delta = g_pos - g_neg
...
model = nn.Sequential(
    nn.Sequential(LeakyLayer(700, n_units)),
...
def print_evaluation(epoch=None):
    ...
    print(f"[{epoch_str}] Training Acc: {(1-train_error)*100:>5.2f}%\t Test Acc: {(1-test_error)*100:>5.2f}%")
...
def test_rec(epoch=None):
    ...
    train_final_list['epoch'].append(epoch)
    train_final_list['train_acc'].append(round(((1-train_error)*100),2))

    test_final_list['epoch'].append(epoch)
    test_final_list['test_acc'].append(round(((1-test_error)*100),2))

    save_list_to_txt(train_final_list['train_acc'], train_acc_path)
    save_list_to_txt(test_final_list['test_acc'], test_acc_path)
...
for epoch in range(num_epochs):
    ...
    print(sum(tensor.sum() for tensor in spk1_rec_global))
    if (epoch + 1) % 1 == 0:
