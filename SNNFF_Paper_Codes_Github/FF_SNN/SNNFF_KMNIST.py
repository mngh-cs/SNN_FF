...
class LeakyLayer2(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        ...
        self.lif2 = snn.Leaky(beta=beta, threshold=threshold, spike_grad=spike_grad4, learn_threshold=True, learn_beta=True)
        self.layer_outputs = []
        self.num_steps = num_steps
...
train_set = KMNIST('./data/', train=True, download=True, transform=transform)
test_set = KMNIST('./data/', train=False, download=True, transform=transform)
...
x_te = torch.load('./data/KMNIST/baked/test_x.pt', device)
y_te = torch.load('./data/KMNIST/baked/test_y.pt', device)
...
def average_chunks(input_list, target_size):
    chunk_size = len(input_list) // target_size
    result = []

    with open('loss_KMNIST.txt', 'w') as f:

        for i in range(target_size):
            chunk = input_list[i * chunk_size : (i + 1) * chunk_size]
            average = sum(chunk) / len(chunk)
            result.append(round(average.item(), 3))
            f.write(str(round(average.item(), 3)))
            f.write('\n')
...
for epoch in range(num_epochs):
    ...
    average_chunks(loss_dict[0], epoch+1)
    test_rec(epoch)
